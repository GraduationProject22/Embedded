...
void CAN_voidConfigFilter(CAN_FilterTypeDef *pFilterConfig)
{
	...
	if (pFilterConfig->FilterScale == CAN_FILTERSCALE_16BIT)
	{
		...
		CAN->FilterBankRegister[pFilterConfig->FilterBank].FR1 = (((pFilterConfig->FilterMaskIdLow & 0x0000FFFF) << 16) |
																  (pFilterConfig->FilterIdLow & 0x0000FFFF));

		/* Second 16-bit identifier and Second 16-bit mask */
		/* Or Third 16-bit identifier and Fourth 16-bit identifier */
		CAN->FilterBankRegister[pFilterConfig->FilterBank].FR2 = (((pFilterConfig->FilterMaskIdHigh & 0x0000FFFF) << 16) |
	...
	else if (pFilterConfig->FilterScale == CAN_FILTERSCALE_32BIT)
	{
		...
		CAN->FilterBankRegister[pFilterConfig->FilterBank].FR1 = (((pFilterConfig->FilterIdHigh & 0x0000FFFF) << 16) |
																  (pFilterConfig->FilterIdLow & 0x0000FFFF));
			...
			else
			{
				if (GET_BIT(tsrflags, (local_u8TxBitPos + 2)) != 0) /* ALST: Arbitration lost for mailbox [0:2] */
				{
					...
					case 0:
						...
						break;
					case 1:
						CAN_ErrorCode |= CAN_ERROR_TX_ALST1;
						break;
					case 2:
						CAN_ErrorCode |= CAN_ERROR_TX_ALST2;
						break;
					}
				}
				else if (GET_BIT(tsrflags, (local_u8TxBitPos + 3)) != 0) /* TERR: Transmission error of mailbox [0:2] */
				{
					...
					case 0:
						...
						break;
					case 1:
						CAN_ErrorCode |= CAN_ERROR_TX_TERR1;
						break;
					case 2:
						CAN_ErrorCode |= CAN_ERROR_TX_TERR2;
						break;
					}
				}
